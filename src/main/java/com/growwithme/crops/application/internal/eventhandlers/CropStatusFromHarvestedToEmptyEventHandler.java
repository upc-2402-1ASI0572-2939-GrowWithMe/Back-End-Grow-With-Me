...
@Service
@AllArgsConstructor
public class CropStatusFromHarvestedToEmptyEventHandler {
    private final CropRepository cropRepository;

    @EventListener(CropStatusFromHarvestedToEmptyEvent.class)
    public void on(CropStatusFromHarvestedToEmptyEvent event) {
        ...
        if (!oldCropStatus.equals(CropStatus.HARVESTED)) {
            throw new IllegalStateException("Crop status must be HARVESTED to transition to EMPTY.");
        } else{
